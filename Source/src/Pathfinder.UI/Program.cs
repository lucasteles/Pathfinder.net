...
namespace Pathfinder.CLI.UI
{
    public class Program
    {
        public static int Main(string[] args)
            => Parser.Default
                .ParseArguments<MapGenerationVerb>(args)
                .MapResult(
                      (MapGenerationVerb opt) => RunMapGeneration(opt),
                      errs => 1
                );


        public static int RunMapGeneration(MapGenerationVerb options)
        {
        ...
        static void GenerateMap(IMapGenerator generator, MapGenerationVerb options)
        {
